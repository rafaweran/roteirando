...
const TourAttendanceView: React.FC<TourAttendanceViewProps> = ({
  ...
  const totalGroups = attendingGroups.length;
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        >
          ...
          Voltar para {trip.name}
        </button>
        
        <div className="flex gap-2">
           <Button variant="outline" className="h-10 text-xs sm:text-sm px-3 sm:px-4">
        ...
        <div className="p-5 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
           ...
           <div className="relative w-full sm:w-72">
              ...
              <input 
                ...
                className="w-full h-10 pl-10 pr-4 rounded-lg border border-border text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                value={searchTerm}
                          ...
                          <td className="py-4 px-6 align-top">
                             <div className="flex flex-wrap gap-2">
                                {group.attendingNames.map((name, idx) => (
                                   <span key={idx} className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-surface border border-border text-text-primary">
                                      {name}
                                   </span>
                                ))}
                             </div>
